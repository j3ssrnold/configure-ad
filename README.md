...
<h3>Step 1: To get started with an on-premises Active Directory within Azure Virtual Machines (VM), begin by creating the Domain Controller VM.</h3>

-  Go to the [Azure Portal](https://portal.azure.com) --> VM Tab --> Click Create Machine --> Enter Resource Group Name as **AD-Lab** --> Name the VM as **DC-1** --> Select **Windows Server 2022 Datacenter**.     
...
<h3>Step 2: Create the Client VM. Go back to the Virtual Machines Tab and click Create Azure Machine.</h3>
...
<h3>Step 3: Set the Domain Controller’s IP to static for easy device location and process efficiency.</h3>
...
<h3>Step 4: Connect to Client-1. VM</h3>
...
<h3>Step 5: Ensure Connectivity Between Client-1 and Domain Controller.</h3>

- Go down to Search bar and type in **cmd**. Open Command Prompt. Perpetual ping DC-1's private IP address ***10.0.0.4***.
...
-  Log in to your DC-1 VM using Remote Desktop Connection --> Enter the username/password you selected during creation --> Select OK.
...
-  Your DC-1 homescreen display should look like this:
...
<h3>Step 6: Next you will want to install Active Directory on your Domain Controller.</h3>
...
<h3>Step 7: Promoting the Server to a Domain Controller.</h3>
...
-  A configuration wizard pops up. You will need to add a new forest as **mydomain.com**. (You may name it whatever you wish).
...
<h3>Step 8: Add an Admin and Normal Users in Active Directory.</h3>
...
-  Create an Admin. --> Click _ADMINS folder --> right click New --> Add User --> Enter in the information for (Jane Doe), your Admin --> Click Next.
...
<h3>Step 9: Join Client-1 to your domain (mydomain.com).</h3>
...
-  Log in to Client-1 as labuser (original local admin) --> Settings --> Rename this pc (advanced).
...
-  Login to DC-1 as with Jane using mydomain.com\jane_admin. Verify that Client-1 shows up in Active Directory Users and Computers (ADUC).
   -  Tools --> ADUC --> mydomain.com --> Computers.
        - Then under mydomain.com --> New OU --> _CLIENTS --> Select Client-1 in ***Computers*** --> Move to _CLIENTS for organization.
...
<h3>Step 10: Setup Remote Desktop for Normal Users on Client-1</h3>

-  Log in to Client-1 using the admin account: mydomain.com\jane_admin. Open System Properties.
...
<h3>Step 11:  Finally, use Poweshell to facilitate Mass Account Creation and log-in as a random user.</h3>
